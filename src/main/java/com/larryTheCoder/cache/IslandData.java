...
public class IslandData implements Cloneable {
    ...
    private IslandData(String levelName, String plotOwner, Vector2 gridPos, Vector3 spawnPos, int plotSize, int homeId, int islandUniquePlotId, String islandName,int isUpSize) {
        this.levelName = levelName;
    ...
    private IslandData(Row islandObj, Row dataObj) {
        ...
        this.isUpSize = islandObj.getInteger("isUpSize");
        ...
        this.Locked = dataObj.getBoolean("locked");
    ...
    public boolean checkIsUpSize(int x,int z){
        return x >= (getCenter().getFloorX() - isUpSize/2)
                && x <= (getCenter().getFloorX() + isUpSize/2)
                && z >= (getCenter().getFloorX() - isUpSize/2)
                && z <= (getCenter().getFloorY() + isUpSize/2);
    }
        ...
        ASkyBlock.get().getDatabase().pushQuery(new DatabaseManager.DatabaseImpl() {
            ...
            public void executeQuery(Connection connection) {
                connection.createQuery(TableSet.ISLAND_UPDATE_MAIN.getQuery())
                        ...
                        .addParameter("isUpSize",isUpSize)
                        .executeUpdate();
