...
public class DeleteIslandTask implements Runnable {
    ...
    public DeleteIslandTask(ASkyBlock plugin, IslandData pd, String player) {
        this.plugin = plugin;
        this.pd = pd;
        this.player = player;
    }
    ...
    public void run() {
        ...
        if (pd.onIsland(p)) {
            Server.getInstance().dispatchCommand(p, "hub"); // Easy
        }
        ...
        boolean result = ASkyBlock.get().getFastCache().deleteIsland(pd);
        if (!result) {
            Utils.send("Unable to delete player island data from database");
        }
    }
